...
if(router.get('/user/signup:email(/[a-zA-Z0-9+_.-]+@[a-zA-Z0-9.-]+$/)', (req, res) => {
...
//phone
// /[+]\d{3}? \d{9}$/
if(router.get('/user/signup:phoneNumber(/([+]\d{3}?\d{9}$)/', (req, res) => {
...
// /(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/
if(router.get('/user/signup:password(/(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$)/)', (req, res) => {
