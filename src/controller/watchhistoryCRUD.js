...
const __addWatchhistory = async (req, res, next) => {
  try {
    ...
    if (history) return res.status(200).json({ msg: "this is already added" });

    let createdWatchhistory = await WatchhistoryDB.create({
      ...req.body,
      userID: req.user.id,
    });
...
const __isCompleted = async (req, res, next) => {
  try {
    ...
    if (history.isComplete) return next();
...
const __updateWatchhistory = async (req, res, next) => {
  try {
    ...
    if (history) {
      ...
      console.log(
        "Hassan ~ file: watchhistoryCRUD.js ~ line 84 ~ history.videos.length <= count.count",
        history.videos.length <= count.count
      );
      ...
      if (history.videos.length >= count.count) {
        ...
        return res
          .status(200)
          .send("You alreay watch all video in this Course");
      } else {
        await WatchhistoryDB.update(
          {
            ...history,
      ...
      history.videos.forEach((element) => {
        if (req.body.video === element) flag = false;
      });
      ...
      if (flag) {
        ...
        console.log(
          "Hassan ~ file: watchhistoryCRUD.js ~ line 89 ~ data",
          videos
        );
      ...
      if (flag) return res.status(200).json({ updated });
